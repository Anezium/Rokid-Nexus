...
class TransitPlugin(
    private val nearMeForeground: (Boolean) -> Boolean = { false },
) : NexusPlugin {
    ...
    private lateinit var runtime: TransitRuntime
    ...
    private val runtimeHost = object : TransitRuntimeHost {
                    ...
                    .put(
                        ...
                        JSONArray().also { lines ->
                            card.lines.forEach { line ->
                                lines.put(
                                    if (line.badge.isBlank() && line.trail.isEmpty()) {
                                        line.text
                                    } else {
                                        JSONObject()
                                            .put("text", line.text)
        ...
        override fun setNearMeForeground(active: Boolean): Boolean = nearMeForeground(active)
    }

    override fun onRegister(host: NexusPluginHost) {
        this.host = host
        val favorites = TransitFavoritesStore(host.context)
        runtime = TransitRuntime(
            host = runtimeHost,
            dependencies = TransitDependencies(
                repository = TransitRepository(),
                location = TransitLocationProvider(host.context),
                favorites = favorites,
            ),
        )
    ...
    private companion object {
        ...
        const val PLUGIN_PREFIX = "/plugin/transit"
