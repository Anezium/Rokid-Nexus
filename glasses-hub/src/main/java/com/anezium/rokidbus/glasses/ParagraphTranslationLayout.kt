...
internal const val FROZEN_MIN_READABLE_TEXT_SP = 6.25f
...
internal data class ParagraphPanelCandidateQuality(
...
internal data class ParagraphRenderedTextMetrics(
    val truncated: Boolean,
    val retainedTextFraction: Float,
)

internal fun paragraphRenderedTextMetrics(
    textLength: Int,
    lastLineStart: Int,
    lastLineEnd: Int,
    ellipsisStart: Int,
    ellipsisCount: Int,
): ParagraphRenderedTextMetrics {
    if (textLength <= 0) return ParagraphRenderedTextMetrics(false, 1f)
    val retainedEnd = if (ellipsisCount > 0) {
        lastLineStart + ellipsisStart
    } else {
        lastLineEnd
    }.coerceIn(0, textLength)
    val truncated = ellipsisCount > 0 || lastLineEnd < textLength
    return ParagraphRenderedTextMetrics(
        truncated = truncated,
        retainedTextFraction = if (truncated) retainedEnd.toFloat() / textLength else 1f,
    )
}
...
internal fun generateParagraphPanelGrowthCandidates(
    ...
    return capDiverseParagraphPanelCandidates(ordered, candidateLimit)
}

/**
 * Keeps bounded work without letting max-area envelopes consume every slot. Width, height, mixed
 * growth, and zero-growth terminal shapes all receive deterministic representation when available.
 */
private fun capDiverseParagraphPanelCandidates(
    ordered: List<ParagraphPanelGrowthCandidate>,
    candidateLimit: Int,
): List<ParagraphPanelGrowthCandidate> {
    if (ordered.size <= candidateLimit) return ordered
    val selected = LinkedHashSet<ParagraphPanelGrowthCandidate>()
    fun addFirst(predicate: (ParagraphPanelGrowthCandidate) -> Boolean) {
        ordered.firstOrNull(predicate)?.let(selected::add)
    }

    addFirst(ParagraphPanelGrowthCandidate::isZeroGrowth)
    addFirst(ParagraphPanelGrowthCandidate::isWideGrowth)
    addFirst(ParagraphPanelGrowthCandidate::isTallGrowth)
    addFirst(ParagraphPanelGrowthCandidate::isMixedGrowth)
    ordered.forEach { candidate ->
        if (selected.size < candidateLimit) selected += candidate
    }
    return selected.take(candidateLimit)
}

/** Exhausts candidates and ranks text quality before geometry; truncation is terminal-only. */
internal fun <T> selectParagraphPanelGrowthFit(
    ...
    quality: (T) -> ParagraphPanelCandidateQuality,
): T? = candidates.mapIndexedNotNull { index, candidate ->
    prepareCandidate(candidate)?.let { prepared ->
        ScoredParagraphPanelCandidate(prepared, quality(prepared).sanitized(), candidate, index)
    }
}.maxWithOrNull(paragraphPanelCandidateComparator)?.value

private data class ScoredParagraphPanelCandidate<T>(
    val value: T,
    val quality: ParagraphPanelCandidateQuality,
    val candidate: ParagraphPanelGrowthCandidate,
    val index: Int,
)

private val paragraphPanelCandidateComparator =
    compareBy<ScoredParagraphPanelCandidate<*>> { it.quality.complete }
        .thenBy { it.quality.retainedTextFraction }
        .thenBy { it.quality.textSizePx }
        .thenByDescending { it.quality.panelAreaPx }
        .thenBy { it.candidate.totalGrowthAreaProxy() }
        .thenBy { it.candidate.growLeftPx + it.candidate.growRightPx }
        .thenBy { it.candidate.growUpPx + it.candidate.growDownPx }
...
private fun ParagraphPanelGrowthCandidate.isWideGrowth(): Boolean =
    growLeftPx + growRightPx > 0f && growUpPx + growDownPx == 0f

private fun ParagraphPanelGrowthCandidate.isTallGrowth(): Boolean =
    growLeftPx + growRightPx == 0f && growUpPx + growDownPx > 0f

private fun ParagraphPanelGrowthCandidate.isMixedGrowth(): Boolean =
    growLeftPx + growRightPx > 0f && growUpPx + growDownPx > 0f

private fun ParagraphPanelGrowthCandidate.totalGrowthAreaProxy(): Float =
    (growLeftPx + growRightPx + 1f) * (growUpPx + growDownPx + 1f)

private fun ParagraphPanelCandidateQuality.sanitized() = copy(
...
private fun horizontalBandsOverlap(
    ...
    val overlap = min(firstRight, secondRight) - max(firstLeft, secondLeft)
    if (overlap <= 0f) return false
    val narrowerWidth = min(firstRight - firstLeft, secondRight - secondLeft)
    return overlap + FIT_EPSILON_PX >= narrowerWidth * VERTICAL_BLOCKER_MIN_HORIZONTAL_OVERLAP_FRACTION
}
...
private const val VERTICAL_BLOCKER_MIN_HORIZONTAL_OVERLAP_FRACTION = 0.35f
